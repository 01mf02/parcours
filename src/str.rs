...
pub fn matches<'a, 'i: 'a, S>(x: &'a str) -> impl Parser<&'i str, S, O = ()> + 'a {
    FromFn(move |input: &'i str, _state: &mut S| input.strip_prefix(x).map(|rest| ((), rest)))
}
